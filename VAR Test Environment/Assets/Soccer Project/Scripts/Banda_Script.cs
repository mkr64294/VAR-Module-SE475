...
public class Banda_Script : MonoBehaviour {
	...
	public Vector3 direction_saque;
	...
	void OnTriggerEnter( Collider other) {
		...
		if ( other.gameObject.tag == "Ball" && Camera.main.GetComponent<InGameState_Script>().state == InGameState_Script.InGameState.PLAYING ) {
			...
			Camera.main.GetComponent<InGameState_Script>().state = InGameState_Script.InGameState.SAQUE_BANDA;
			Camera.main.GetComponent<InGameState_Script>().postionBanda = sphere.gameObject.transform.position;		
